...
```{r}
dataset <- readr::read_csv("results/full_data_noise.csv") %>% dplyr::filter(task=="image")
...
probsuccess.fit$save_object(file = "./probsuccess-fit-image.RDS")

```

```{r}
probsuccess.fit <-readRDS("./probsuccess-fit-image.RDS")
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior effects (image data)', 
      booktabs=T,
      label='tab:diagnostics-image-probsuccess',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-image.tex')
```
...
colnames(tt) <- c("Parameter", "Mean", "HPD low", "HPD high",'OR HPD low','OR Mean','OR HPD high')
 ...
 saveRDS(tt,'./probsuccess-par-table-image.RDS')
```
...
kable(
        ...
        booktabs=T,
        label='summarystatisticstable-OR-image',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-OR-image.tex')
```

Statistical Analysis
```{r}
p_trace <- bayesplot::mcmc_trace(draws_a, pars = a_alg)
png("traceplots-probsuccess-image.png")
p_trace
```
...
png("posterior-parameters-grouped-probsuccess-image.png")
p_bars_grouped
...
Posterior Predictive Check
```{r}
p_bars_grouped_prior <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=dataset$algorithm, binwidth = 0.005)
#pdf('posterior-predictive-check-bars_grouped-aggregated.pdf')
png("prior-parameters-grouped-probsuccess-image.png")
