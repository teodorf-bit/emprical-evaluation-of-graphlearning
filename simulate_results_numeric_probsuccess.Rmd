...
```{r}
dataset <- readr::read_csv("results/full_data_noise.csv") %>% dplyr::filter(task=="numeric")
...
file <- file.path(cmdstanr::cmdstan_path(), "probsuccess-numeric.stan")
...
probsuccess.fit$save_object(file = "./probsuccess-fit-numeric.RDS")

```

```{r}
probsuccess.fit <-readRDS("./probsuccess-fit-numeric.RDS")
...
summary_probsuccess.fit <- probsuccess.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior effects (numeric data)', 
      booktabs=T,
      label='tab:diagnostics-numeric-probsuccess',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-probsuccess-numeric.tex')
```
...
colnames(tt) <- c("Parameter", "Mean", "HPD low", "HPD high",'OR HPD low','OR Mean','OR HPD high')
 ...
 saveRDS(tt,'./probsuccess-par-table-numeric.RDS')
```
...
kable(
        "latex", 
        table.envir = 'table',
        caption='Odds ratios for fixed effects and noise parameters (numeric data)', 
        booktabs=T,
        label='summarystatisticstable-OR-numeric',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-OR-numeric.tex')
```

Statistical Analysis
```{r}
p_trace <- bayesplot::mcmc_trace(draws_a, pars = a_alg)
png("traceplots-probsuccess-numeric.png")
p_trace
```
...
# png("posterior-parameters-grouped-probsuccess-numerical.png")
# p_bars_grouped
...
#pdf('posterior-predictive-check-bars_grouped-aggregated.pdf')
png("prior-parameters-grouped-probsuccess-numerical.png")
