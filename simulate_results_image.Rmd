...
dataset%>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error rate (image data)', 
        booktabs=T,
        label='image',
        format.args = list(scientific = FALSE), 
        digits = 3,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-accuracy-image.tex')
```
...
pdf("boxplots-accuracy-image.pdf")
#png("boxplots-accuracy-image.png")
p
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the accuracy (image data)', 
        booktabs=T,
        label='summarystatisticstable-image',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-image.tex')
```
...
saveRDS(df_out, file="./ranking-image.RDS")
```

```{r pressure, echo=FALSE}
df_out <- readRDS("./ranking-image.RDS")
...
saveRDS(standata, file = "./rankingmodel-data-image.RDS")
```
CmdStanR
```{r}
standata<-readRDS("./rankingmodel-data-image.RDS")
...
saveRDS(ranking.fit, file = "./ranking-fit-image.RDS")
ranking.fit$save_object(file = "./ranking-fit-image.RDS")

```
Read in the fitted data
```{r}
ranking.fit <-readRDS("./ranking-fit-image.RDS")
...
summary_ranking.fit <- ranking.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (image data)', 
      booktabs=T,
      label='tab:diagnostics-image',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-image.tex')
```
...
# #pdf("parcoord-image.pdf")
# png("parcoord-image.png")
# p_parcoord
```
Kernel density estimates
```{r}
# p_dense_overlay <-bayesplot::ppc_dens_overlay(y,as.matrix(y_rep))
# #pdf("dense_overlay-image.pdf")
# png("dense_overlay-image.png")
# p_dense_overlay
```
...
#pdf('strength-image.pdf')
png("strength-image.png")
p_mcmc_intervals
```
...
#pdf('posterior-strength-parameters-image.pdf')
png("posterior-strength-parameters-image.png")
p_areas
```

```{r}
# p_bars <- bayesplot::ppc_bars(y=y,yrep=as.matrix(y_rep))
# #pdf('posterior-predictive-check-boxplot-image.pdf')
# png("posterior-predictive-check-boxplot-image.png")
# p_bars
```

```{r}
p_bars_grouped <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=df_out$algo0_name, binwidth = 0.005)
png("posterior-parameters-grouped-image.png")
p_bars_grouped
...
png("prior-parameters-grouped-image.png")
...
# #pdf('posterior-predictive-check-rootogram-image.pdf')
# png("posterior-predictive-check-rootogran.image.png")
# p_rootogram
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms (image data)', 
      booktabs=T,
      label='rankingimage',
      format.args = list(scientific = FALSE), 
      digits = 3,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('ranking-image.tex')
```
