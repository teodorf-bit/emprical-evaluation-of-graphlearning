...
dataset%>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the error rate (text data)', 
        booktabs=T,
        label='text',
        format.args = list(scientific = FALSE), 
        digits = 3,
    ...
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-accuracy-text.tex')
```
...
pdf("boxplots-accuracy-text.pdf")
#png("boxplots-accuracy-text.png")
p
...
dataset %>% 
  ...
  kable(
        "latex", 
        table.envir = 'table',
        caption='Summary statistics for the accuracy (text data)', 
        booktabs=T,
        label='summarystatisticstable-text',
        format.args = list(scientific = FALSE), 
        digits = 3,
        linesep = "") %>% 
    kable_styling(latex_options = c("hold_position"),
                  full_width = F) %>% 
    readr::write_lines('summarystatisticstable-text.tex')
```
...
saveRDS(df_out, file="./ranking-text.RDS")
```

```{r pressure, echo=FALSE}
df_out <- readRDS("./ranking-text.RDS")
...
saveRDS(standata, file = "./rankingmodel-data-text.RDS")
```
CmdStanR
```{r}
standata<-readRDS("./rankingmodel-data-text.RDS")
...
saveRDS(ranking.fit, file = "./ranking-fit-text.RDS")
ranking.fit$save_object(file = "./ranking-fit-text.RDS")

```
Read in the fitted data
```{r}
ranking.fit <-readRDS("./ranking-fit-text.RDS")
...
summary_ranking.fit <- ranking.fit$summary(variables = a_alg) %>%
  ...
  kable(
      "latex", 
      table.envir = 'table',
      caption='Diagnostics for the posterior ranks (text data)', 
      booktabs=T,
      label='tab:diagnostics-text',
      format.args = list(scientific = FALSE), 
      digits = 3,
      linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('diagnostics-text.tex')
```
...
# #pdf("parcoord-text.pdf")
# png("parcoord-text.png")
# p_parcoord
```
Kernel density estimates
```{r}
# p_dense_overlay <-bayesplot::ppc_dens_overlay(y,as.matrix(y_rep))
# #pdf("dense_overlay-text.pdf")
...
#pdf('strength-text.pdf')
png("strength-text.png")
p_mcmc_intervals
```
...
#pdf('posterior-strength-parameters-text.pdf')
png("posterior-strength-parameters-text.png")
p_areas
```

```{r}
# p_bars <- bayesplot::ppc_bars(y=y,yrep=as.matrix(y_rep))
# #pdf('posterior-predictive-check-boxplot-text.pdf')
# png("posterior-predictive-check-boxplot-text.png")
# p_bars
```

```{r}
p_bars_grouped <- bayesplot::ppc_stat_grouped(y=y, yrep=as.matrix(y_rep), group=df_out$algo0_name, binwidth = 0.005)
png("posterior-parameters-grouped-text.png")
p_bars_grouped
...
#pdf('posterior-predictive-check-bars_grouped-text.pdf')
png("prior-parameters-grouped-text.png")
...
# #pdf('posterior-predictive-check-rootogram-text.pdf')
# png("posterior-predictive-check-rootogran.text.png")
# p_rootogram
...
rank_df_table %>% 
  kable(
      "latex", 
      table.envir = 'table',
      caption='Ranking of the algorithms (text data)', 
      booktabs=T,
      label='rankingtext',
      format.args = list(scientific = FALSE), 
      digits = 3,
  ...
  kable_styling(latex_options = c("hold_position"),
                full_width = F) %>% 
  readr::write_lines('ranking-text.tex')
```
